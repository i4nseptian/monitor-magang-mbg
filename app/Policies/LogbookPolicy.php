...
class LogbookPolicy
{
    ...
    public function view(User $user, Logbook $logbook): bool
    {
        if ($user->hasRole('mahasiswa')) {
            return $user->id === $logbook->user_id && $user->can('view_logbook');
        }
    ...
    {
        if ($user->hasRole('mahasiswa')) {
            return $user->id === $logbook->user_id && $user->can('update_logbook');
        }
    ...
    {
        if ($user->hasRole('mahasiswa')) {
            return $user->id === $logbook->user_id && $user->can('delete_logbook');
        }
