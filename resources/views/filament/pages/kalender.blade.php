    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
            <h2 class="flex items-center gap-2 text-base font-bold text-gray-900 sm:text-lg dark:text-white">
                <x-filament::icon icon="heroicon-o-calendar-days" class="h-5 w-5 shrink-0 text-primary-600 dark:text-primary-400" />
        ...
        <div class="hidden overflow-hidden rounded-xl border border-gray-200 bg-white sm:block dark:border-gray-700 dark:bg-gray-900">
            <div class="grid grid-cols-7 border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-950/50">
                @foreach ($weeks as $dayName)
                    <div class="py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        {{ $dayName }}
                    </div>
                @endforeach
            </div>
            <div class="grid grid-cols-7 auto-rows-[minmax(100px,auto)] divide-x divide-y divide-gray-100 dark:divide-gray-800">
                @foreach ($days as $day)
                    ...
                    @endphp
                    <div class="{{ $dayClass }} transition-all duration-200 hover:shadow-md hover:z-10 hover:scale-[1.02]">
                        <div class="mb-1 flex items-center justify-between">
                            <span @class([
                                'flex h-6 w-6 items-center justify-center text-xs font-bold',
                                'rounded-full bg-primary-600 dark:bg-primary-500 text-white' => $day['is_today'],
                            ])>{{ $day['date']->day }}</span>
                        ...
                        <div class="flex-1 space-y-0.5 overflow-y-auto">
                            ...
                            <div class="truncate rounded border border-primary-100 bg-primary-50/60 px-1.5 py-[2px] text-[10px] font-medium text-primary-800 transition-all duration-200 hover:border-primary-300 hover:shadow-sm dark:border-primary-900 dark:bg-primary-950/40 dark:text-primary-300 dark:hover:border-primary-700"
                                 title="{{ $log->judul_kegiatan }} ({{ $log->user->name }})">
                                    {{ Str::limit($log->judul_kegiatan, 18) }}
                                </div>
                            @endforeach
                            @foreach ($day['documentations'] as $doc)
                            <div class="truncate rounded border border-gray-200 bg-gray-50 px-1.5 py-[2px] text-[10px] text-gray-600 transition-all duration-200 hover:border-gray-300 hover:shadow-sm hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:border-gray-600 dark:hover:bg-gray-700/50"
                            ...
                            <div @class([
                                ...
                                <div class="mb-2 flex items-center justify-between">
                                    ...
                                    @if($day['logbooks']->count() > 0 || $day['documentations']->count() > 0)
                                        <span class="rounded bg-primary-100 px-2 py-0.5 text-[10px] font-semibold tabular-nums text-primary-700 shadow-sm dark:bg-primary-950 dark:text-primary-300">
                                ...
                                @else
                                    <div class="space-y-1.5">
                                        ...
                                        @endforeach
                                        @foreach ($day['documentations'] as $doc)
                                            <div class="flex items-start gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 transition-all duration-200 hover:border-gray-300 hover:shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600">
                                                <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full bg-gray-400 dark:bg-gray-500"></span>
                                                <div class="min-w-0 flex-1">
                                                    <p class="text-xs font-medium text-gray-700 dark:text-gray-300">📷 {{ $doc->judul }}</p>
                                                    <p class="text-[10px] text-gray-400 dark:text-gray-500">{{ $doc->user->name }}</p>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
        ...
        <div class="flex flex-wrap items-center gap-4 rounded-xl border border-gray-200 bg-gradient-to-r from-gray-50/90 to-white p-4 text-xs dark:border-gray-700 dark:from-gray-900/60 dark:to-gray-900/20">
                            <span class="font-semibold text-gray-700 dark:text-gray-300">Legenda:</span>
            <div class="flex items-center gap-1.5">
                <span class="inline-block h-2.5 w-2.5 rounded-sm bg-primary-600 dark:bg-primary-500"></span>
                <span class="text-gray-600 dark:text-gray-400">Logbook</span>
            </div>
            <div class="flex items-center gap-1.5">
                <span class="inline-block h-2.5 w-2.5 rounded-sm bg-gray-400 dark:bg-gray-500"></span>
                <span class="text-gray-600 dark:text-gray-400">Dokumentasi</span>
            </div>
            <div class="flex items-center gap-1.5">
                <span class="flex h-5 w-5 items-center justify-center rounded-full bg-primary-600 text-[9px] font-bold text-white dark:bg-primary-500">H</span>
