    ...
    <div class="space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8">
            ...
            <x-stat-card label="Total Project" :value="$finalStats['total_project']" color="success" 
                icon='<svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 13.5h3.86a2.25 2.25 0 012.008 1.24l.885 1.77a2.25 2.25 0 002.007 1.24h1.98a2.25 2.25 0 002.007-1.24l.885-1.77a2.25 2.25 0 012.007-1.24h3.86m-18 0h18m-18 0v-7.5A2.25 2.25 0 014.25 3.5h15.5A2.25 2.25 0 0122 5.75v7.5m-18 0v5.25c0 .621.504 1.125 1.125 1.125h13.5c.621 0 1.125-.504 1.125-1.125v-5.25"/></svg>' />
            <x-stat-card label="Evaluasi Mentor" :value="$finalStats['total_evaluasi']" color="info" 
                icon='<svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 8.25h9m-9 3h9m-9 3h9m-16.5 5.25h15.75a2.25 2.25 0 002.25-2.25V4.5A2.25 2.25 0 0017.25 2.25H3.75A2.25 2.25 0 001.5 4.5v12.75a2.25 2.25 0 002.25 2.25z"/></svg>' />
            <x-stat-card label="Skill Terdata" :value="$finalStats['total_skills']" color="primary" 
                icon='<svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 21l8.904-4.43c2.32-.477 4.1-2.4 4.093-4.771V6.518c0-.987-.777-1.802-1.76-1.849a48.514 48.514 0 00-6.19-.24c-2.42 0-4.8.31-7.098.913a1.875 1.875 0 00-1.397 1.826v6.096c0 .882.35 1.728.974 2.353l3.93 3.93A1.875 1.875 0 009.813 15.904z"/></svg>' />
            ...
            <x-stat-card label="Rasio Kehadiran" :value="$finalStats['persen_hadir'] . '%'" color="warning" 
                icon='<svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6a7.5 7.5 0 107.5 7.5h-7.5V6z"/><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0013.5 3v7.5z"/></svg>' />
        </div>

        {{-- Ringkasan Magang — Metronic clean card --}}
        <div class="rounded-xl border border-[oklch(94%_0.004_286.32)] dark:border-[oklch(22%_0.01_260)] bg-white dark:bg-[#161920] shadow-[0_1px_2px_rgb(0_0_0/0.05)]">
            <div class="border-b border-slate-100 dark:border-slate-800 px-5 py-3.5">
                <h3 class="flex items-center gap-2 text-sm font-semibold text-slate-800 dark:text-slate-200">
                    <div class="flex h-7 w-7 items-center justify-center rounded-lg bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400">
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div class="rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 p-4">
                        <p class="text-xs font-medium text-slate-400 dark:text-slate-500">Periode Magang</p>
                        <p class="mt-1.5 text-sm font-semibold text-slate-800 dark:text-slate-200 tabular-nums">
                            {{ $finalStats['tgl_mulai']->format('d M Y') }} – {{ $finalStats['tgl_selesai']->format('d M Y') }}
                        </p>
                    </div>
                    <div class="rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 p-4">
                        <p class="text-xs font-medium text-slate-400 dark:text-slate-500">Total Hari</p>
                        <p class="mt-1.5 text-sm font-semibold text-slate-800 dark:text-slate-200 tabular-nums">
                            {{ $finalStats['total_hari'] }} Hari Kerja
                        </p>
                    </div>
                    <div class="rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 p-4">
                        <p class="text-xs font-medium text-slate-400 dark:text-slate-500">Kategori Terbanyak</p>
                        <p class="mt-1.5 text-sm font-semibold text-slate-800 dark:text-slate-200">
                    ...
                    <div class="rounded-lg border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 p-4">
                        <p class="text-xs font-medium text-slate-400 dark:text-slate-500">Total Dokumentasi</p>
                        <p class="mt-1.5 text-sm font-semibold text-slate-800 dark:text-slate-200 tabular-nums">
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <div class="rounded-xl border border-[oklch(94%_0.004_286.32)] dark:border-[oklch(22%_0.01_260)] bg-white dark:bg-[#161920] p-5 shadow-[0_1px_2px_rgb(0_0_0/0.05)]">
                <h4 class="mb-4 text-xs font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-2">
            ...
            <div class="md:col-span-2 rounded-xl border border-[oklch(94%_0.004_286.32)] dark:border-[oklch(22%_0.01_260)] bg-white dark:bg-[#161920] p-5 shadow-[0_1px_2px_rgb(0_0_0/0.05)]">
                <h4 class="mb-4 text-xs font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-2">
                    <span class="kt-dot kt-dot-primary"></span>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                                ...
                                <th class="px-4 py-3 text-right text-xs font-semibold text-slate-500 dark:text-slate-400">Jumlah Kontribusi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            @foreach($finalStats['category_distribution'] as $cat)
                                <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/30">
                                    ...
                                    <td class="px-4 py-3 text-right font-semibold tabular-nums text-indigo-600 dark:text-indigo-400">
                                        {{ $cat['total'] }}
                                    </td>
                                </tr>
                            @endforeach
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderCharts() {
                ...
                const defaults = {
                    ...
                    plugins: {
                        legend: { labels: { boxWidth: 10, padding: 12, font: { size: 11, weight: '500' }, color: fontColor } }
                    },
                ...
                if (skillLabels.length) {
                    new Chart(document.getElementById('skillChart'), {
                        ...
                        data: {
                            labels: skillLabels,
                            datasets: [
                                { label: 'Awal', data: @json($skillData['awal']), backgroundColor: dark ? '#312e81' : '#e0e7ff', borderRadius: 4 },
                                { label: 'Akhir', data: @json($skillData['akhir']), backgroundColor: dark ? '#6366f1' : '#4f46e5', borderRadius: 4 }
                            ]
                        },
